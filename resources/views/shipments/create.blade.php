...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="m-0"><i class="bi bi-plus-circle me-2"></i>Add Shipment</h2>
            ...
            <form action="{{ route('shipments.store') }}" method="POST">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="cargo_type" class="form-select" required>
                            ...
                            @foreach($cargoTypes as $type)
                                <option value="{{ $type }}" {{ old('cargo_type') == $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="shipment_status" class="form-select" required>
                            @foreach($statuses as $status)
                                <option value="{{ $status }}" {{ old('shipment_status', 'Planning') == $status ? 'selected' : '' }}>
                                    {{ $status }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="cargo_description" class="form-control" rows="3"
                                  placeholder="Optional description of cargo contents…">{{ old('cargo_description') }}</textarea>
                    </div>
                ...
                <div class="mt-4 text-end">
                    ...
                    <button type="submit" class="btn btn-primary px-4">
                        <i class="bi bi-save me-1"></i> Save Shipment
                    </button>
                </div>
            </form>
        </div>
