        ...
        <form method="POST" action="{{ route('admin.articles.store') }}">
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-12">
                    ...
                    <input type="text" name="title" class="form-control" value="{{ old('title') }}" placeholder="{{ __('Example: Impact of Red Sea Crisis...') }}" required>
                </div>
                
                <div class="col-md-6">
                    <label class="form-label">{{ __('Category') }}</label>
                    <input type="text" name="category" class="form-control" value="{{ old('category') }}" placeholder="{{ __('Example: Shipping, Economy, Weather') }}">
                </div>

                <div class="col-md-6">
                    <label class="form-label">{{ __('Cover Image URL (Optional)') }}</label>
                    <input type="url" name="image" class="form-control" value="{{ old('image') }}" placeholder="https://contoh.com/gambar.jpg">
                </div>

                <div class="col-md-12">
                    <label class="form-label">{{ __('Tags (Comma separated)') }}</label>
                    <input type="text" name="tags" class="form-control" value="{{ old('tags') }}" placeholder="{{ __('Example: Red Sea, Logistics, Crisis') }}">
                </div>

                <div class="col-md-12">
                    <label class="form-label">{{ __('Article Content') }}</label>
                    <textarea name="content" class="form-control" rows="8" placeholder="{{ __('Write your analysis here...') }}" required>{{ old('content') }}</textarea>
                </div>
