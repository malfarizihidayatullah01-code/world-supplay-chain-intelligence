...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="m-0"><i class="bi bi-graph-up-arrow me-2"></i>Route Risk Analysis Detail</h2>
        <div>
            <form action="{{ route('route-risk-analysis.analyse', $analysis->shipment_id) }}" method="POST" class="d-inline">
                @csrf
                <button type="submit" class="btn btn-outline-primary me-2">
                    <i class="bi bi-arrow-repeat"></i> Re-analyse
                </button>
            </form>
            <a href="{{ route('route-risk-analysis.index') }}" class="btn btn-outline-secondary">
        ...
        <div class="col-md-4">
            <div class="card shadow-sm border-0 h-100" style="border-radius: 16px;">
                <div class="card-header bg-white border-bottom p-4" style="border-top-left-radius: 16px; border-top-right-radius: 16px;">
                    <h5 class="mb-0 fw-bold">Overall Risk Score</h5>
                </div>
                <div class="card-body p-4 text-center d-flex flex-column justify-content-center">
                    <div class="display-1 fw-bold mb-2">{{ number_format($analysis->route_score, 1) }}</div>
                    <div>
                    ...
                    <div class="row g-4">
                        <div class="col-md-4">
                            <div class="border rounded p-3 text-center">
                                <div class="text-muted small mb-1">Origin Country Risk</div>
                                <div class="fs-3 fw-semibold">{{ number_format($analysis->origin_country_risk, 1) }}</div>
                                <div class="text-muted small">{{ $shipment->originCountry->country_name ?? 'N/A' }}</div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="border rounded p-3 text-center">
                                <div class="text-muted small mb-1">Destination Country Risk</div>
                                <div class="fs-3 fw-semibold">{{ number_format($analysis->destination_country_risk, 1) }}</div>
                                <div class="text-muted small">{{ $shipment->destinationCountry->country_name ?? 'N/A' }}</div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="border rounded p-3 text-center">
                                <div class="text-muted small mb-1">Weather Risk</div>
                                <div class="fs-3 fw-semibold">{{ number_format($analysis->weather_risk, 1) }}</div>
                                <div class="text-muted small">Origin & Destination Avg</div>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="border rounded p-3 bg-light">
                                <div class="fw-bold mb-2">Analysis Notes:</div>
                                <p class="mb-0 text-muted">{{ $analysis->analysis_notes ?: 'No specific notes.' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
